...
Read(/{image})` pre-approves reading the one staged file and nothing else: in the
...
`.env` gets that read denied, not answered. Claude Code reads files inside
its working directory with no rule and no prompt, so the run's working
directory is the staged file's own temporary folder, which holds nothing
else, whatever folder melampus was launched from (from Lightroom that is
the app's own). The staged path is handed over
