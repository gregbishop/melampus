...
def ollama_answers(url: str | None = None) -> bool:
    ...
    non-200: unavailable. Never raises; a probe reports."""
    try:
        with urllib.request.urlopen(
            f"{ollama_url(url)}/api/version", timeout=OLLAMA_PROBE_SECONDS
        ) as response:
...
def default_engine(ollama_at: str | None = None) -> str:
    ...
    return next((v.engine for v in detect_engines(ollama_at) if v.available), "mlx")
