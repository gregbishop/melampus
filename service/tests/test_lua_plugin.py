...
from test_binary import _no_python_environment, _per_user_data_dir, per_user_config
...
def _plugin_folder_holding(executable: Path, tmp_path: Path) -> Path:
    ...
    installed: copied, as the user copies it there from the download."""
...
def _quoted_for_the_shell(word: str) -> str:
    """`word` as the plugin quotes it for the shell of the Lightroom it fakes
    here: double quotes for cmd.exe, single quotes for sh."""
    return f'"{word}"' if WINDOWS else f"'{word}'"
...
def test_the_engine_preference_reaches_the_executable_through_the_command_the_plugin_builds(
    ...
    assert f"--backend {_quoted_for_the_shell('ollama')}" in command, command
...
def test_the_log_lands_under_the_data_directory_the_executable_reports(
    ...
    assert data_root == _per_user_data_dir(Path(env["HOME"]))
...
def test_the_cli_engine_preference_reaches_the_executable_through_the_command_the_plugin_builds(
    ...
    assert f"--backend {_quoted_for_the_shell(engine)}" in command, command
    ...
    assert line.startswith(_quoted_for_the_shell(str(plugin_dir / built_executable.name))), (
...
def test_the_stored_key_reaches_the_executable_through_the_command_the_plugin_builds(
    ...
    assert f"--backend {_quoted_for_the_shell('openai')}" in command, command
    arguments = command.split(str(plugin_dir / built_executable.name), 1)[1]
