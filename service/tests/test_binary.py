...
def test_executable_refuses_ollama_when_no_server_answers(
    ...
    env = _no_python_environment(tmp_path)
    proc = subprocess.run(
        [str(built_executable), str(photos), "--backend", "ollama",
...
def test_executable_refuses_claude_code_that_is_not_installed(
    ...
    proc = subprocess.run(
        [str(built_executable), str(photos), "--backend", "claude-code",
         "--cache", str(tmp_path / "cache.jsonl")],
        env=env, capture_output=True, text=True, timeout=600,
    )
    tail = proc.stderr[-3000:]
...
def test_executable_downloads_the_model_from_the_hub_with_no_python_on_the_path(
    ...
    env = _no_python_environment(tmp_path) | hub_env
    proc = subprocess.run(
    ...
    tail = proc.stderr[-3000:]
    for missing in ("ModuleNotFoundError", "ImportError"):
        assert missing not in tail, f"the executable does not carry the hub library:\n{tail}"
