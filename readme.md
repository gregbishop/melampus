...
and so is the engine when the process that started melampus ignores SIGCHLD,
...
the signal's default in the launcher).
...
`--backend command` (with `[model] command` set). With no `--backend`
